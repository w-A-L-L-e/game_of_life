...
void Screen::rectangle(Uint32 x, Uint32 y, Uint32 width, Uint32 height) {
  line(x,y, x+width, y);
...
void Screen::fill_rectangle(Uint32 x, Uint32 y, Uint32 width, Uint32 height) {
  // TODO: bounds check (if outside screen dont bother to draw etc...)
  for(Uint32 ypos=y; ypos < y+height; ypos++){
    for(Uint32 xpos=x; xpos < x+width; xpos++){
      pixel(xpos, ypos);
    }
  }
} 
